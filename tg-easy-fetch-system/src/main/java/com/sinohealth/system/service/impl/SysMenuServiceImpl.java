...
import com.sinohealth.common.core.domain.TreeSelect;
...
import com.sinohealth.system.domain.vo.MetaVo;
import com.sinohealth.system.domain.vo.RouterVo;
import com.sinohealth.system.mapper.SysMenuMapper;
...
public class SysMenuServiceImpl extends ServiceImpl<SysMenuMapper, SysMenu> implements ISysMenuService {
     ...
     */
    @Override
    public List<SysMenu> selectMenuList(Long userId,Integer roleType) {
    ...
    public List<SysMenu> selectMenuList(SysMenu menu, Long userId) {
       ...
       List<Long> roleIds= roleService.selectRoleListByUserId(userId);
        if (roleIds.stream().anyMatch(id->id<3L)) {
            menuList = list(new LambdaQueryWrapper<SysMenu>()
                    ...
                    .eq(StrUtil.isNotBlank(menu.getMenuType()),SysMenu::getMenuType,menu.getMenuType())
                    .eq(menu.getMenuClassify() !=null,SysMenu::getMenuClassify,menu.getMenuClassify())
     ...
     */
    @Override
    public List<RouterVo> buildMenus(List<SysMenu> menus,boolean isVueRouter) {
        ...
        for (SysMenu menu : menus) {
            ...
            router.setMenuCode(menu.getMenuCode());
            List<SysMenu> cMenus = menu.getChildren();
            boolean isMenu=isVueRouter?(UserConstants.TYPE_DIR.equals(menu.getMenuType())||UserConstants.TYPE_MENU.equals(menu.getMenuType())):UserConstants.TYPE_DIR.equals(menu.getMenuType());
            if (!cMenus.isEmpty() && cMenus.size() > 0 && isMenu) {
                router.setAlwaysShow(true);
                if(!isVueRouter) {
                    router.setRedirect("noRedirect");
                }
                router.setChildren(buildMenus(cMenus,isVueRouter));
                ...
                children.setMenuCode(menu.getMenuCode());
