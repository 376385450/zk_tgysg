 ...
 *
 * @author linkaiwei
 * @date 2021/10/19 09:53
 * @since 1.4.5.0
 */
public class HiveUtils {

    private static final String driverName = DatabaseEnum.HIVE2.getDriver();
    private static final String url = "jdbc:hive2://192.168.52.21:10000/sinohealth_test?";
...
//        }
//        //
//        res = state.executeQuery("desc formatted sinohealth_test.test_create_table01");
//        while (res.next()) {
...
//        table.setTableName("sinohealth_test.test_create_table05");
...
//        table.setPartitions(Lists.newArrayList(
//                new TableField().setFieldName("sex").setDataType("INT")));
...
//        table1.setTableName("sinohealth_test.test_create_table01");
//        table1.setFields(Arrays.asList(
//                new TableField().setFieldName("id").setDataType("INT").setFieldAlias("ID").setValue("4"),
//                new TableField().setFieldName("name").setDataType("STRING").setFieldAlias("名称").setValue("燕六"),
...
//        table2.setTableName("sinohealth_test.test_create_table01");
...
//        table3.setTableName("sinohealth_test.test_create_table01");
...
//        // 查询表信息
//        final String descFormattedTable = descFormattedTable("sinohealth_test.test_01");
...
//        }
    ...
    public static String createTable(Table table) {
        ...
        if (partitions != null && !partitions.isEmpty()) {
            ...
            partitions.forEach(partition -> sql.append(String.format("%s %s,", partition.getFieldName(),
    ...
    public static String descFormattedTable(String tableName) {
        ...
        return String.format("DESC FORMATTED %s", tableName);
//        return String.format("DESCRIBE FORMATTED %s", tableName);
    ...
    public static String insert(Table table) {
        ...
        // 字段值
        fields.forEach(field -> sql.append("'").append(field.getValue()).append("',"));
        sql.deleteCharAt(sql.length() - 1).append(")");
...
//     * @since 1.4.5.0
//     */
//    public static String query(String tableName, String fieldName, String condition, String leftVal, String rightVal,
//                               String fieldIds, String sortBy, String sortingField) {
//        if (tableName == null || tableName.trim().length() < 1) {
//            return null;
//        }
//        StringBuilder sql = new StringBuilder();
...
//
//        // 过滤字段信息
//        if (fieldName != null && fieldName.trim().length() > 0 && condition != null && condition.trim().length() > 0
//                && leftVal != null && leftVal.trim().length() > 0
//                || "null".equals(condition) || "notNull".equals(condition)) {
//
//
//        }
//
